...
---
As MASTER CLAUDE, help operate this system for: $ARGUMENTS
...
1. **Where are we?** — `ops-env-map` first, always. It writes `.mc/env`. If you cannot *prove* the environment, it is production.
...
Findings become issues via `mc-issues` (local files stay canon; security is held back on public repos).
