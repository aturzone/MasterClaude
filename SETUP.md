...
SKULL is a set of Claude Code **skills, agents, and commands** as plain `.md` files. You set it up
...
> **If you are Claude Code reading this to set SKULL up:** follow
> [For Claude Code / AI agents](#for-claude-code--ai-agents) at the bottom — clone the repo and copy
> `skills/`, `agents/`, `commands/` into the project's `.claude/`, then load the `skull` skill.
...
## Verify it worked
- In Claude Code, run `skull` (or `/skull`) — the leader should greet you and start the
  interview.
- `/skull-team` reports the assembled team; `/skull:whats-new` checks versions/changelog;
  `/skull:security` runs a security pass; `/sentinel:map` builds the project map.
...
rm -rf /tmp/mc
```
Or just ask SKULL to **"update yourself"** and it'll do exactly this.

## Uninstall
Delete what you copied:
```bash
rm -rf .claude/skills/skull .claude/skills/{planning,review,understand,guardrails,security,workflows} \
       .claude/agents/sentinel.md .claude/agents/security .claude/commands/skull* .claude/commands/sentinel \
...
A user pointed you here and asked to "use SKULL." Set it up like this:

1. Confirm `claude --version` ≥ **2.1.183** (tell the user to `claude update` if older).
2. `git clone https://github.com/aturzone/Skull` into a temp dir (e.g. `/tmp/mc`).
...
7. Load and run the **`skull`** skill (`.claude/skills/skull/SKILL.md`) — it interviews the
