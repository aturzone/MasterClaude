...
MASTER CLAUDE's QA team. Point it at any project and it stands up a **per‑project test workspace**,
then runs every kind of test — the way a real user experiences the app, from the outside as a black
box, down in the code, and under load — and writes a single charted status dashboard (`mc.html`) at
...
- **`wf-tester`** — the driver. Maps the project, scaffolds/confirms the QA workspace, runs the four
  passes, and generates `mc.html`. Start here (or say "test everything" / run `/master-claude:test`).
...
- **`mc-dashboard`** — renders `mc.html`: a CLI‑styled, professional, charted view of MASTER CLAUDE's team and its findings for this project.
- Agent: **`tester`** (`agents/testing/tester.md`) — read‑only toward source; drives the engine, writes results under `.mc/qa/` + the root `mc.html`.
