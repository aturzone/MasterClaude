#!/usr/bin/env node
// ui-intel — SKULL's design-intelligence query engine.
...
// `over` carries deliberate overrides ({palette, fonts, style}). The database is a starting point,
// not a mandate: a real brief ("make it read academic, not like developer docs") outranks a
// product-type default every time. What matters is that the override is RECORDED as one, so the
// next reader knows a human chose it and doesn't "fix" it back to the default.
function designSystem(query, stack, over = {}) {
  const product = search("products", query, 1)[0]?.item;
  if (!product) {
    return { unmatched: true, query, suggestions: load("products").slice(0, 8).map((p) => p.name) };
  }
  const overrides = [];
  const pick = (domain, chosen, fallback, label) => {
    if (!chosen || chosen === fallback) return byId(domain, fallback);
    const hit = byId(domain, chosen);
    if (!hit) die(`unknown ${domain} "${chosen}". Try: node uikit.mjs --list ${domain}`);
    overrides.push(`${label}: **${hit.name}** (the database would have picked ${byId(domain, fallback)?.name || fallback})`);
    return hit;
  };
  const palette = pick("palettes", over.palette, product.palette, "Palette");
  const fonts = pick("fonts", over.fonts, product.fonts, "Typography");
  const styleIds = over.style ? String(over.style).split(",").map((s) => s.trim()) : product.style || [];
  if (over.style) overrides.push(`Style: **${styleIds.join(" + ")}** (the database would have picked ${(product.style || []).join(" + ")})`);
  const styles = styleIds.map((s) => byId("styles", s)).filter(Boolean);
  return { product, styles, palette, fonts, rules: checklist(load("ux-rules")), stack, overrides };
}
...
function renderDesignSystem(ds, projectName) {
  ...
  const { product, styles, palette, fonts, rules, stack, overrides } = ds;
  const L = [];
  L.push(`# Design system — ${projectName || product.name}`);
  L.push(``);
  L.push(`> Generated by SKULL \`ui-intel\`. This is the **source of truth** for this project's UI.`);
  L.push(`> Page-specific overrides live in \`pages/<page>.md\` and win over this file.`);
  L.push(``);
  if (overrides?.length) {
    L.push(`## ⚠ Deliberate overrides`);
    L.push(`A human chose these against the database's default. **Do not "correct" them back** — the brief`);
    L.push(`outranks a product-type default, and that is the whole reason this section exists.`);
    L.push(``);
    for (const o of overrides) L.push(`- ${o}`);
    L.push(``);
  }
  L.push(`| | |`);
  L.push(`|---|---|`);
  ...
  L.push(``);
  L.push(`---`);
  L.push(`*SKULL · ui-intel. Regenerate with \`--force\`; hand-edits are kept otherwise.*`);
  return L.join("\n");
}
...
function persist(ds, { out, projectName, page, force }) {
  ...
  const dir = path.join(root, ".skull", "design");
...
const USAGE = `ui-intel — SKULL design intelligence

  node uikit.mjs --design-system "<query>" [--stack <s>] [-p "<name>"] [--persist --out <root>] [--page <p>] [--force]
       …override the database's pick when the brief says otherwise (it is recorded as an override):
       [--palette <id>] [--fonts <id>] [--style <id>[,<id>]]
...
function main(argv) {
  ...
  if (has("--design-system")) {
    ...
    const ds = designSystem(q, flag("--stack"), {
      palette: flag("--palette"), fonts: flag("--fonts"), style: flag("--style"),
    });
    if (has("--persist")) {
