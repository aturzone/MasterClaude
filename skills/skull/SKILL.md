...
| a tool would clearly help — big repo, token cost, PII heading to a model, stale library docs | **secret-shop** | scan → recommend the right *external* tool (graphify/caveman/Context7/Presidio…) → record it → use it |
...
| the deliverable is a document — a PDF report, a Word/Excel/PowerPoint file, an invoice, a form | **doc-pdf / doc-office** | the right library per format; correct, styled, accessible output |
