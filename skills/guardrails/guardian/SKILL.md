...
2. **Tests are sacred.** Never weaken, skip, comment out, delete, or loosen an assertion to make a suite go green. If a test fails, fix the cause or report it honestly. *(This is a discipline you hold — but teams that want it enforced rather than merely followed can arm the optional `guardian-test-guard` PreToolUse hook, which blocks silently skipping or weakening a test at the tool call; see [docs/ENFORCEMENT.md](../../../docs/ENFORCEMENT.md).)*
